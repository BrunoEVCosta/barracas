...
def generateTable(table,output):
  ...
  for attribute, datatype in tableAttributes[1::]:
    attr={'attribute':attribute,'datatype':datatype.replace("int","INTEGER").replace("varchar","STRING").replace("date","DATE").replace("text","STRING").replace("float","FLOAT").replace("year","INTEGER")}
